...
Phases 0–5 are done (foundation, player shooting, enemies/collision/damage, bullet-hell
emitters + respawn i-frames, the full stage/arcade loop, and polish: title/gameplay screen
split, pause, music flow, menu/graze SFX, banking frames, explosion sparks, difficulty
tuning). Post-v1 candidates are in PLAN.md.

- Entry point: `Program.cs`
- Main game class: `Game1.cs` — owns device setup, shared content/services, and the screen manager
...
- **Factory ↔ system wiring gotcha:** `EntityFactory` needs the built `World`, which doesn't exist until `WorldBuilder.Build()`. Systems that spawn or react (`WeaponSystem`, `EnemySpawnSystem`, `EmitterSystem`, `DamageSystem`) therefore can't take the factory/services as constructor args — `GameplayScreen.NewGame()` builds the world, then sets them via public fields. Follow that pattern for new spawning systems. (System update order is set by `AddSystem` order in `GameplayScreen`; see PLAN.md §3.)
- **Use ECS where it makes sense (PLAN.md §6).** Per-entity gameplay → components/systems. Global/singleton state → plain classes in `Source/`, injected into systems: `SfxManager` (one-shot SFX) and `MusicManager` (looping music via `MediaPlayer`) — audio is split by API/lifecycle; gameplay systems only fire SFX (field named `Sfx`), while music is driven by screens, never systems. Also `AnimationLibrary` (shared animation clips), `PlayerTracker` (the player's position, for systems that aim at it), `GameState` (arcade flow phase + global score) and `Stage` (the wave timeline `EnemySpawnSystem` plays back). Don't force those into the ECS. Gameplay systems gate on `GameState.Phase == GamePhase.Playing` so the world freezes on pause / STAGE CLEAR / GAME OVER; `GameplayScreen.NewGame()` rebuilds the world to (re)start a run.
